...
async def get_calories(
    ...
    """
    try:
        logger.info(f"Calorie lookup request: {request.dish_name} x {request.servings} for user {current_user.email}")
        ...
        if not food_data:
            ...
            return JSONResponse(
                status_code=404,
                content={"detail": f"Dish '{request.dish_name}' not found in food database"}
            )
        
        # Calculate calories per serving (assuming standard 100g serving)
        # In a real implementation, you might want more sophisticated serving size handling
        calories_per_100g = food_data["calories_per_100g"]
        
        # For simplicity, assume 1 serving = 100g
        # This can be enhanced later with actual serving size data
        ...
        return JSONResponse(
            status_code=500,
            content={"detail": "Internal server error while processing calorie request"}
        )
