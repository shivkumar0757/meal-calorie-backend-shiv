...
class USDAService:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                params = {
                    ...
                    "pageSize": 3,  # Reduce to top 3 results for efficiency
                    "dataType": ["Foundation", "SR Legacy", "Branded"]  # Allow branded for better coverage
                }
            ...
            raise HTTPException(
                status_code=503, 
                detail="External food database temporarily unavailable"
            )
        except httpx.RequestError as e:
            logger.error(f"USDA API request error: {e}")
            raise HTTPException(
                status_code=503, 
                detail="Unable to connect to food database"
            )
        except Exception as e:
            logger.error(f"Unexpected error in USDA service: {e}")
            raise HTTPException(
                status_code=500, 
                detail="Internal error while fetching food data"
            )
    
    def _find_best_food_match(self, foods: list, query: str) -> Optional[Dict[str, Any]]:
        """
        ...
        def sort_key(food):
            ...
            return (priority, -score)  # Lower priority number = better, higher score = better
    ...
    def calculate_serving_calories(self, calories_per_100g: int, serving_size_g: float = 100) -> int:
        """
