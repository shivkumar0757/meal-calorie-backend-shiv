...
class Settings(BaseSettings):
    ...
    def effective_database_url(self) -> str:
        ...
        if self.database_url:
            ...
            if self.environment == Environment.DEV:
                ...
                if self.database_url.startswith("sqlite"):
                    ...
                    return "postgresql://meal_user:dev_password@localhost:5432/mealcalorie"
                return self.database_url
        ...
        logger.info(f"Using {self.environment.upper()} profile database: {url.split('@')[0]}@***")
        return url
...
logger.info(f"🗄️  Database: {'SQLite' if settings.effective_database_url.startswith('sqlite') else 'PostgreSQL'}")
